...
#define DP_NUM_LMT  30
#define SN_LEN_LMT  32
...
typedef struct {
    ...
    CHAR firm_key[PRODUCT_KEY_LEN+1]; // add by mjl 20191217 
    DEV_ABI_E ability;
    BOOL bind;
    BOOL sync;
}DEV_DESC_IF_S;
...
typedef struct {
	...
	#if (ESP8266_1M != 1)
	CHAR chip_mac[MAC_ADDR_LEN+1]; //2M原厂mac备份,验证其变化所用, 入网采用mac为上面mac变量
	#endif
	CHAR psk_key[PSK_KEY_LEN+1];
    CHAR auz_key[AUTH_KEY_LEN+1]; //authorize key
    CHAR fac_pin[FAC_PIN_LEN+1];
