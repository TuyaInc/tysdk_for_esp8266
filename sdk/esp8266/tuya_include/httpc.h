...
typedef enum  {
    ...
    HDR_ADD_TY_PRIV_HEAD = 0x0040, // add by nzy 20161202
    HRD_ADD_DOWNLOAD_RANGE = 0x0080, /* add downlaod offset */
...
//typedef void * http_session_t;  in realtek
// user http head add callback
typedef void (*HTTP_HEAD_ADD_CB)(http_session_t session);

#if 1
#define PRIVATE_HDR_FLAGS (HDR_ADD_TY_PRIV_HEAD) // add by nzy 20161202
#else
#define PRIVATE_HDR_FLAGS HDR_ADD_DEFAULT_USER_AGENT
#endif
...
#endif
{
    ...
    #else
    int rc = http_open_session(handle, hostname, flags, retry_cnt);
    #endif
    if (WM_SUCCESS != rc)
        return rc;
