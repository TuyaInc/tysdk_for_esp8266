...
输入参数: mthd
           ...
           2    特殊配网模式(special with low power)
 		   3    兼容模式 old mode scan
		   4    低功耗自动模式 with low power mode && auto cfg
           5    特殊配网自动模式 special with low power mode && auto cfg
          callback
...
备    注: 只支持字符串类型
*************************************************************************************/
OPERATE_RET tuya_set_prod_test_scan_ssid(IN         CHAR *ssid);

/*************************************************************************************
函数功能: 设置复位回调
输入参数: cbs      复位回调函数
返 回 值: 参照返回值列表
备    注: GW_RESET_CB 对应app移除设备    GW_RESET_FAC_CB 对应恢复出厂设置
*************************************************************************************/
OPERATE_RET tuya_set_gw_reset_cb(TY_RESET_CBS* cbs);
